...
const InteriumReportList: React.FC<{ yearRange: string, language: string }> = ({ yearRange, language }) => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        const result = await getInteriumReportCollections(language);
        console.log("Results");
        console.log(result);
  ...
  const cmsBaseUrl = process.env.NEXT_PUBLIC_CMS_URL || 'http://52.175.21.181'; // Example: http://localhost:1337
