...
export default async function Foundations({ searchParams }: FoundationsPageProps) {

  let foundationsSectionsData: SectionItem[] = []; // Added to store fetched foundations sections
  const awaitedSearchParams = await searchParams; // Await searchParams
  const selectedCollections = awaitedSearchParams?.collections || '';

  try {
    const foundationsResponse = await getFoundation();
    if (foundationsResponse) {
      foundationsSectionsData = foundationsResponse.data.Sections.sections;
    }
  } catch (error) {
        ...
        <div className="container mx-auto pl-64">
          <div className="grid grid-cols-1 md:grid-cols-">
            ...
            <div className="md:col-span-2">
              {selectedCollections === '' ? (
                <DynamicBusinessList collections="foundations-collections" key="foundations-collections" />
              ) : (
