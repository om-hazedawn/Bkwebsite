...
export default function MarineWorks() {
  ...
  const [marineWorksSectionsData, setMarineWorksSectionsData] = useState<SectionItem[]>([]);
  const searchParams = useSearchParams();
  const selectedCollections = searchParams.get('collections');

  useEffect(() => {
    const fetchMarineWorksData = async () => {
      const marineWorksResponse = await getMarineWork(language);
      if (marineWorksResponse) {
        setMarineWorksSectionsData(marineWorksResponse.data.Sections.sections);
      }
    };
    fetchMarineWorksData();
  }, [language]);
        ...
        <div className="relative max-w-[1690px] w-full h-[400px] sm:h-[500px] md:h-[640px] lg:h-[740px] mx-auto overflow-hidden">
          ...
          <div className="absolute inset-0">
            <div className="hidden min-[750px]:grid grid-cols-3 grid-rows-2 gap-0 w-full h-full max-w-[1110px] mx-auto">
              ...
              <div className="relative w-full h-full flex items-end pl-8 pb-10 bg-black/70 text-white">
                <h3 className="text-xl sm:text-2xl lg:text-3xl font-semibold">Marine Works</h3>
              ...
              <Link href="/our-business">
                <div className="relative w-full h-full flex items-end pl-8 pb-10 bg-[#D5D4D5] text-[#595957] transition-colors duration-300 hover:bg-[#D5D4D5]/60">
                  <h3 className="text-lg sm:text-xl lg:text-[1.50rem] font-semibold">◄ back to Our Business</h3>
        ...
        <div className="container mx-auto pl-64">
          <div className="grid grid-cols-1 md:grid-cols-3">
            <div className="md:col-span-1">
              {/* Tunnels section */}
              <h2 className="text-2xl font-medium text-gray-700 mb-6">Marine Works</h2>
