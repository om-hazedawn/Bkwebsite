...
export default function CivilWorks() {
  ...
  const [civilWorksSectionsData, setCivilWorksSectionsData] = useState<SectionItem[]>([]);
  const searchParams = useSearchParams();
  const selectedCollections = searchParams.get('collections');

  useEffect(() => {
    const fetchCivilWorksData = async () => {
      const civilWorksResponse = await getGeneralCivilWork(language);
      if (civilWorksResponse) {
        setCivilWorksSectionsData(civilWorksResponse.data.Sections.sections);
      }
    };
    fetchCivilWorksData();
  }, [language]);
        ...
        <div className="relative max-w-[1690px] w-full h-[400px] sm:h-[500px] md:h-[640px] lg:h-[740px] mx-auto overflow-hidden">
          ...
          <div className="absolute inset-0">
            <div className="hidden min-[750px]:grid grid-cols-3 grid-rows-2 gap-0 w-full h-full max-w-[1110px] mx-auto">
              ...
              <div className="relative w-full h-full flex items-end pl-8 pb-10 text-white bg-black/60">
                <h3 className="text-xl sm:text-2xl lg:text-3xl font-semibold">General Civil Works</h3>
              </div>
