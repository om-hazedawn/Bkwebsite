...
export default function Tunnels() {
  ...
  const [tunnelSectionData, setTunnelSectionData] = useState<SectionItem[]>([]);
  ...
  useEffect(() => {
    const fetchTunnelsData = async () => {
      ...
      if (tunnelResponse) {
        setTunnelSectionData(tunnelResponse.data.Sections.sections);
      }
    };
    fetchTunnelsData();
  }, [language]);
        ...
        <div className="relative max-w-[1690px] w-full h-[400px] sm:h-[500px] md:h-[640px] lg:h-[740px] mx-auto overflow-hidden">
          ...
          <div className="absolute inset-0">
            <div className="hidden min-[750px]:grid grid-cols-3 grid-rows-2 gap-0 w-full h-full max-w-[1110px] mx-auto">
              ...
              {/* Second box - Tunnels */}
              <div className="relative w-full h-full flex items-end pl-8 pb-10 bg-white/70 text-black">
                <h3 className="text-xl sm:text-2xl lg:text-3xl font-semibold">Tunnels</h3>
              </div>
