export async function getMarineWorksDetail() {
    const CMS_URL = process.env.CMS_URL || 'https://bk-data-migrate.onrender.com';
    const API_TOKEN = process.env.STRAPI_API_TOKEN;
  
    try {
      const res = await fetch(`${CMS_URL}/api/marine-works-detail?populate=*`, {
        method: 'GET',
      ...
      if (!res.ok) {
        ...
        throw new Error(`Failed to fetch marine-works-detail data. Status: ${res.status}`);
      }
  
      const data = await res.json();
      return data;
    } catch (error) {
      console.error('Error fetching marine-works-detail data:', error);
