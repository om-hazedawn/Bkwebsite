export async function getOtherCollections() {
    ...
    try {
      const res = await fetch(`${CMS_URL}/api/others-collections?populate=*`, {
        method: 'GET',
      ...
      if (!res.ok) {
        ...
        throw new Error(`Failed to fetch others-collections data. Status: ${res.status}`);
      }
  
      const data = await res.json();
      return data;
    } catch (error) {
      console.error('Error fetching others-collections data:', error);
