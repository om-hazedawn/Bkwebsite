export async function getSportgroundsAndParksCollections() {
    const CMS_URL = process.env.CMS_URL || 'https://bk-data-migrate.onrender.com';
    const API_TOKEN = process.env.STRAPI_API_TOKEN;
  
    try {
      const res = await fetch(`${CMS_URL}/api/sportgrounds-and-parks-collections?populate=*`, {
        method: 'GET',
      ...
      if (!res.ok) {
        ...
        throw new Error(`Failed to fetch sportgrounds-and-parks-collections data. Status: ${res.status}`);
      }
  
      const data = await res.json();
      return data;
    } catch (error) {
      console.error('Error fetching sportgrounds-and-parks-collections data:', error);
